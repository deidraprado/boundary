	...
	for _, name := range flagsVaultSshCertificateMap[c.Func] {
		...
		case keyBitsName:
			f.UintVar(&base.UintVar{
				...
				Usage:  "The number of bits when generating the ssh private key.",
			})
		case ttlName:
			f.StringVar(&base.StringVar{
				Name:   ttlName,
...
func (c *VaultSshCertificateCommand) extraVaultSshCertificateHelpFunc(_ map[string]func() string) string {
	...
	case "create":
		helpStr = base.WrapForHelpText([]string{
			...
			"",
			`    $ boundary credential-libraries create vault-ssh-certificate -credential-store-id csvlt_1234567890 -vault-path "/some/path"`,
			"",
			"",
		})
	...
	case "update":
		helpStr = base.WrapForHelpText([]string{
			...
			"",
			`    $ boundary credential-libraries update vault-ssh-certificate -id clvlt_1234567890 -name devops -description "For DevOps usage"`,
			"",
			"",
		})
	}
